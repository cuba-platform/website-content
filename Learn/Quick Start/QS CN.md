...
本快速开始指南将蜻蜓点水地使用 CUBA 开发一个简单、但是功能完备的大会计划应用。它会展现开发任何 web 应用涉及的四大项： 如何设计数据模型，如何操作数据，如果创建业务逻辑，以及如何创建用户界面。 做为示例，我们会创建一个简单、但是功能完备的应用版本，它用来帮助大会做会议计划。 实际上，这个教程对初次开发 CUBA 应用来说足够了，我们现在就开始吧。 这个教程会用到 <a target="_self" href="/tools/">CUBA Studio</a>，所以开始之前请安装好它，并接受一个商业试用版来使用可视化设计器。
...
我们来使用 Intellij IDEA 对应的菜单创建一个空的 CUBA 项目，命名为 SessionPlanner， 项目命名空间为 - planner。我们将使用 Java 11 做为默认 JDK。
...
第一次启动 CUBA Studio, 它为你提供一个 CUBA Studio 的商业试用版。
...
我们来申请一个试用版，它有28天的免费期， 为我们提供一些有用的、可选的设计器。
...
做为开始，我们来创建  ```Speaker``` 实体。我们使用项目欢迎页的链接：
...
或者右键点击屏幕左侧CUBA项目树的 **Data Model** 节点，然后选择 **New -> Entity** 。
...
在CUBA中我们使用标准的 JPA 实体，所以你既可以使用代码编辑器，也可以使用可视化设计器创建实体。只需点击图标 “+” 然后往实体中添加属性，CUBA Studio 会为你生成类成员。
...
在CUBA中，通过 <b>Instance Name（实例名称）</b>，你可以为一个实体指定一个格式化的字符串做为它的显示。对于 speaker， 我们选择姓和名。
...
如果我们看一下实体设计器底部的 **Text** 标签页，会看到一个JPA注解的Java类。需要的话生成的源码可以修改，切换到 <b>Designer</b> 标签页的话，设计器也会包含对应的修改。
...
你也可以看到 DDL 预览，需要的话创建索引。例如，我们预见会有很多针对 last name（姓）的搜索，为了使搜索更有效，我们为last name字段创建一个索引。
...
我们来继续，创建 ```Session``` 实体，然后把它与 ```Speaker``` 关联。字段说明在下方的表格中。 session end time（会议结束时间）是一个计算值，为 开始时间 + 持续时间（start date + duration）。
...
需要添加一个必须的引用到speaker。关系为多对一，所以，我们定义一个关联属性叫做 speaker，引用 speaker 类。最后，字段定义看起来如下：
...
现在，我们需要为会议结束时间创建一个自动计算的属性。首先添加getter方法，命名为```getEndDate```。用```@MetaProperty```注解它。我们还需要为它指定要加载的字段，为 ```startDate``` 和 ```duration```。然后添加计算逻辑。
...
注意Studio高亮了这个方法，因为我们需要为这个属性指定一个文本标签。按下 **Alt+Enter** 然后往信息包中添加一项文本。
...
CUBA Studio包含一个UI界面生成向导，帮助我们创建基础但是有用的UI界面：
...
首先，我们创建speaker的界面。这个实体很简单，我们使用界面创建的默认参数。在实体设计器顶端，点击 **Screens** 菜单下的 **Create Screen** 菜单开始一个向导。
...
你也可以通过右键点击CUBA项目树下的实体，然后在弹出菜单中选择 **New -> Screen** 。
...
对于 ```Speaker``` 实体，我们会创建默认的浏览界面和编辑界面。在向导的<b>Screen Templates</b>标签页选择<b>Entity browser and editor screens</b> 。这个实体简单，我们可以使用界面创建的默认参数。
...
可以看到，每个界面包含两部分： 一个controller，用Java编写，用来处理内部的界面逻辑和事件处理；和一个定义界面外观的XML布局描述。在这个例子中，浏览界面包含 ```speaker-browse.xml``` 和 ```SpeakerBrowse.java```两个文件；相应的，编辑界面包含 ```speaker-edit.xml``` 和 ```SpeakerEdit.java```。源文件的位置在CUBA项目树的 **Generic UI -> Screens** 菜单下。
...
请注意XML界面描述中的 ```data``` 一节 - 它定义数据如何从数据库获取。
...
CUBA平台中，Entity View（实体视图）指定哪些字段需要从数据库读取。你可以在一个独立的文件中定义视图然后在应用的不同模块中使用，也可以在创建界面的时候创建内联视图。
...
另外，为新会议设置默认持续时间为一个小时。要做到这个，在 <b>Component Inspector</b> 窗口的 Handlers 标签页订阅 ```InitEntity``` 事件。
...
选择 <b>CUBA -> Generate Database Scripts</b> 菜单来创建数据库SQL脚本。保存成项目文件之前，你可以在弹出窗口审阅生成的脚本，你也可以在项目树的 <b>Main Data Store</b> 节点下找到它们。
...
如果想增加一些特别的东西，比如额外的索引，或者是初始化数据的insert语句，可以直接修改上述脚本。
...
点击 <b>Create database</b> 按钮应用这些脚本，创建数据库。除了应用表，CUBA会创建系统表，用来保存用户、角色、任务等信息。
...
一会儿你便可以通过浏览器访问应用。 URL地址会显示在IDEA的Run工具箱中。我们这个例子中它是：http://localhost:8080/app 。 在浏览器打开这个URL，然后使用“admin”用户登录应用。密码默认为“admin”。你可以在 **Application** 菜单下找到实体操作界面。
...
然后我们来为数据库添加一些数据：两个speaker，和本周余下时间为他们计划的两个session。你可以尝试为speaker输入不规范的email， 然后会看到email验证如期生效。
...
对于基本操作，生成的界面足够好了，但是实际上，UI往往更复杂。
...
在网格界面之外，我们来为浏览session再添加一个日历界面。 我们为浏览界面添加一个选项卡组件，在上面放一个日历，在之上提供编辑、重新计划会议的的功能，大概界面如下：
...
在设计器中打开```session-browse.xml```文件，在<b>Component Hierarchy</b>窗口把session表格放在tab页中。
...
在 <b>Component Hierarchy</b> 窗口，选择```TabSheet```元素，然后在<b>Component Inspector</b>中选择```expanded```。 Studio会要求一个 ID。 在CUBA的代码中，我们通过ID引用一个界面元素。
...
为标签页指定id： ```calendarTab``` 和 ```tableTab```。然后相应的为他们设置标题： ```Sessions calendar``` 和 ```Sessions table``` 。
...
更新日历 - 分配一个数据容器（data container），使它撑满界面。
...
- ```endDateProperty``` 到session的结束时间（end Date）
- ```captionProperty``` 到session的主题（topic）
- And ```descriptionProperty``` 到session的描述（description）
...
除了使用可视化设计器，你也可以使用XML标记编辑器。我们来添加导航按钮。
...
要看到变化，你不需要重启应用，只需要在应用中重新打开界面。CUBA框架对界面支持热部署。现在你可以看到会议已经在日历中显示了。
...
当我们与UI交互时，会产生事件，因此CUBA为你提供一个API，用来订阅这些事件并处理。我们来处理一个点击日历条目的事件，然后调用session编辑页面。CUBA为界面操作提供一个screen builder API，我们来使用它。
...
在 ```session-browse.xml``` 中选择 ```sessionsCalendar``` ，然后在<b>Component Inspector</b> 窗口选择 Handles标签页。选择 ```CalendarEventClickEvent``` 然后点击箭头来到controller。
...
我们需要调用编辑界面来修改session属性。我们来用 ```EditorScreenFacet``` 。 这是一个可以用来预配置编辑界面的组件。
...
切换到```session-browse.xml```，然后在<b>Component Palette</b>找到<b>EditorScreen</b>，然后把它移到<b>Component Hierarchy</b>窗口的window元素下。
...
回到事件处理，点击窗口上方的**Inject**按钮，然后在弹出窗口的Screen API节选择```sessionEditDialog``` 服务。
...
另一个injection（或者说subscription（订阅））的方法是 - 在编辑页按下 **Alt+Insert**  组合键，然后在弹出菜单选择 **Inject**。
...
要搜索service（服务），只需要键入它的名称然后使用**上** **下**键选择可以注入的服务。
...
为编辑界面传入接收到的事件中的session对象。然后，我们可以展示编辑界面了。
...
好了。你可以在运行的应用中重新打开session浏览界面，然后在日历中通过点击session来调用编辑页面。
...
编辑页面看起来不太好看，我们需要调整一下它的宽高。在IDE中，打开界面的XML描述，选择 ```dialogMode```  属性然后把 ```width``` 和 ```height``` 都设置为 ```auto```。
...
切换到应用程序，关闭编辑界面，然后在日历中点击session重新打开它。现在看起来好很多。
...
现在我们来用CUBA Studio创建一个服务，用来实现业务逻辑，然后在界面中使用它。它是一个session重新计划的服务，并且保证一个speaker在一天之内不超过两个会议。
...
在CUBA项目树中右键点击service节点，选择**New ->Service**。这会打开一个service创建对话框。输入 ```SessionService``` 做为接口名称，实现名称 ``` SessionServiceBean```会为你自动生成。
...
这个方法接受一个session和一个新的session时间做为输入，它会返回更新后的Session实例。
...
要实现这个方法，打开```SessionServiceBean```类的代码编辑器，你可以在CUBA项目树的**Middleware - Services**节点下方找到它。
...
选择```rescheduleSession```方法，如下的代码会生成：
...
对于service，我们会使用CUBA API来访问数据 - ```DataManager```类。我们用这个类创建一个JPQL查询来检查speaker在固定的时间是否已有会议计划，把参数值传入。然后检查查询结果，基于结果，我们为session更新一个新的开始时间或者仅仅把原session实例返回。

Inject the DataManager into the service by pressing **Alt+Insert** in the class body and select **Inject from** the popup menu:
要为service注入DataManager，在类实现中按下 **Alt+Insert**，然后在弹出菜单中选择**Inject from**：
...
service好了，我们来把它加到session浏览界面。它会在日历的拖拽事件中被调用。
...
在```session-browse.xml``` 中选择```sessionsCalendar```，然后在**Component Inspector**窗口切换到 **Handlers** 标签页。选择 ```CalendarEventMoveEvent``` 然后点击箭头来到controller。
...
在订阅拖拽事件的方法中，我们加一些代码来从日历事件中拿到session实体，然后传给service来重新计划这个会议。然后，我们只需要在浏览界面的数据容器（data container）中更新它。
...
为了使新的服务部署，我们需要重启应用程序，还是使用IDEA的  **Run** 按钮。
...
重启后，我们打开会议日历 - 搞定！我们实现了日历的拖拽功能！我们来测试一下，添加额外的会议，然后重新计划它。
...
在CUBA中，你可以更新资源文件然后覆盖标准文本。我们根据应用的业务领域来将它改成 - conference planning。
...
打开主消息包文件 - 它在CUBA项目树的**Generic UI - Main Message Pack**节点下。
...
我们来添加 [Helium](https://www.cuba-platform.cn/marketplace/helium/) 组件。 它是一个新的可视化主题，你可以用它替换标准主题。仅仅点击install然后应用即可。

Now we need to stop the application and update the database to apply the init scripts of the add-on. 
现在我们需要停止应用，然后更新数据库来应用插件的数据库初始化脚本。
...
运行程序，打开settins界面。你会在下拉列表中找到添加的主题，选择它然后应用。
...
CUBA框架提供许多有用的API来帮助你快速创建业务应用。本快速开始指南只展示了基础的CUBA框架和CUBA Studio。你可以在我们的主页：<a target="_self" href="/">cuba-platform.cn</a> 发现更多<a target="_self" href="/learn">示例</a>和<a target="_self" href="/documentation">指南</a>。

Thank you for your interest in CUBA Platform. Enjoy development with CUBA!
感谢你对CUBA平台的关注。享受用CUBA的开发过程吧！